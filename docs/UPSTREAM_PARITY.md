...
| COSMIC session | COSMIC's session manager, compositor, portal, settings, and greeter use vendor archives plus nonstandard install contracts | Tideforge models pinned sources, offline Cargo vendor configurations, native install contracts, and the two EL10 greeter patches for the complete root set. The remaining COSMIC runtime packages must be staged as a complete release set before promotion. | COSMIC login/session smoke |
