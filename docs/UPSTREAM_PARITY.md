...
| COSMIC session | COSMIC's session manager, compositor, and portal use vendor archives plus nonstandard `just`/`make` install contracts | Tideforge models pinned sources, offline Cargo vendor configurations, and native install contracts for `cosmic-session`, `cosmic-comp`, and `xdg-desktop-portal-cosmic`. The remaining COSMIC runtime packages must be staged as a complete release set before promotion. | COSMIC login/session smoke |
