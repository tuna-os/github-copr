...
| COSMIC session | COSMIC's session manager and compositor use vendor archives plus nonstandard `just`/`make` install contracts | Tideforge models pinned sources, offline Cargo vendor configurations, and both install contracts for `cosmic-session` and `cosmic-comp`. The remaining COSMIC runtime packages must be staged as a complete release set before promotion. | COSMIC login/session smoke |
