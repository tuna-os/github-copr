...
| COSMIC session | COSMIC's session manager, compositor, portal, settings, and greeter use vendor archives plus nonstandard install contracts | Tideforge models pinned sources, offline Cargo vendor configurations, native install contracts, and the two EL10 greeter patches for the complete root set. The session manager, compositor, and settings app have real EL10 offline Cargo builds and staged-install proof; their generated RPM payloads are CI-gated. `cosmic-settings-daemon` is now checksum-locked for the next native EL10 closure build. The remaining COSMIC runtime packages must still be staged as a complete release set before promotion. | COSMIC login/session smoke |
