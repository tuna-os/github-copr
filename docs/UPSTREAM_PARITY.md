...
| COSMIC session | COSMIC's session manager uses a vendor archive and nonstandard `just install` contract | Tideforge now models the pinned archive, offline Cargo vendor configuration, source build, and install contract. The remaining COSMIC runtime packages must be staged as a complete release set before promotion. | COSMIC login/session smoke |
