...
| COSMIC session | COSMIC's session manager, compositor, portal, and settings use vendor archives plus nonstandard `just`/`make` install contracts | Tideforge models pinned sources, offline Cargo vendor configurations, and native install contracts for `cosmic-session`, `cosmic-comp`, `xdg-desktop-portal-cosmic`, and `cosmic-settings`. The remaining COSMIC runtime packages must be staged as a complete release set before promotion. | COSMIC login/session smoke |
