...
| XFCE login | XFCE Wayland uses `gtkgreet` hosted by Cage for its graphical greetd login | Tideforge now owns pinned `gtk-layer-shell` and `gtkgreet` recipes. `gtk-layer-shell` has an EL10 staged-install gate; `gtkgreet` promotion remains blocked until that generated `-devel` RPM can be consumed from the factory staging repository. Retain the native spec meanwhile. | `greetd` + Cage + `gtkgreet` login smoke |
