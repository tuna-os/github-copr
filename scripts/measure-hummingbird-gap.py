...
def main() -> None:
    ...
    if args.build_order:
        command = (
            f"scripts/measure-target-gap.py --target {args.target}"
            if args.target
            else f"scripts/measure-hummingbird-gap.py --catalog {catalog_path}"
        )
        emit_build_order(
            args.build_order, catalog, global_tiers, global_cycles, report,
            catalog_path.resolve().parents[1], command=command,
        )
        print(f"wrote {args.build_order}", file=sys.stderr)


def emit_build_order(
    path, catalog, global_tiers, global_cycles, report, root, *, command=None
) -> None:
    ...
    command = command or "scripts/measure-hummingbird-gap.py"
    ...
    lines = [
        ...
        "# graph of every requested desktop at once -- not one per-desktop",
        "# order glued end to end. Minus everything",
        f"# {target['id']} already ships.  Each tier builds in",
        "# parallel; tiers are sequential.  Regenerate with:",
        "#",
        f"#   {command} \\",
        "#     --report-json <report.json> \\",
        "#     --build-order <build-order.yml>",
        "#",
        ...
        "# the accompanying report's buildrequires_cycles. Those",
        ...
        "",
        # A shadow measurement can have no packages. Serialize that as an actual
        # empty YAML sequence, rather than `tiers:` (which YAML reads as null).
        "tiers: []" if not catalog.get("bootstrap") and not global_tiers else "tiers:",
    ]
