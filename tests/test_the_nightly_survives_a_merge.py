...
def test_two_nightlies_share_one_group_so_the_second_queues():
    ...
    Both scheduled runs evaluate the group to the same literal, and with
    cancel-in-progress false GitHub queues the newer one."""
    group = concurrency()["group"]
    assert group.count("nightly") == 1
    assert "github.run_id" not in group and "github.sha" not in group
